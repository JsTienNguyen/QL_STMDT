...
if (isset($_POST['dangki'])) {
    ...
    $pass = md5($pass);
    $reps = md5($repass);
    if (empty($email) || empty($user) || empty($adress) || empty($phone) || empty($storename)) {
    ...
    // "/^\s|\s{2,}|\s$/" trường hợp muốn bỏ đầu và cuối
        ...
        $sql = "INSERT INTO user (`Password`, `user`, `Email`, `Phone`, `Adress`, `RoleID`, `Storename`) 
            VALUES ('$pass', '$user', '$email', '$phone', '$adress', 3, '$storename')";
        ...
        <div class="form-container" style="height: auto;">
            ...
            <form class="form" method="post" action="register.php">
                ...
                <input type="text" name="user"
                    value="<?php echo isset($_POST['user']) ? htmlspecialchars($_POST['user']) : ''; ?>" class="input"
                    placeholder="Tài khoản">
                <input type="password" name="pass"
                    value="<?php echo isset($_POST['pass']) ? htmlspecialchars($_POST['pass']) : ''; ?>" class="input"
                    placeholder="Mật khẩu">
                <input type="password" name="repass" class="input"
                    value="<?php echo isset($_POST['repass']) ? htmlspecialchars($_POST['repass']) : ''; ?>"
                ...
                <input type="text" name="adress" class="input"
                    ...
                    placeholder="Địa chỉ">
                <input type="text" name="storename" class="input"
                    value="<?php echo isset($_POST['storename']) ? htmlspecialchars($_POST['storename']) : ''; ?>"
