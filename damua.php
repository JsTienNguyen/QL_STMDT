  ...
  <li><a href="viewcart.php">Giỏ hàng</a></li>
    ...
    <li><a href="damua.php">Đã mua</a></li>
        ...
        if (isset($_SESSION['client']['user'])) {
            ...
            od.quantity,
            od.price,
            od.Time AS DetailTime,
            od.value,
            od.Description AS DetailDescription,
            od.vote
        FROM 
            oder o
        JOIN 
            oder_deltail od ON o.ID = od.oderID
        WHERE 
            o.Status = 2 AND od.value != 0 AND o.Email = '$email';";
                               ...
                               if($value['discount'] >0)
                                ...
                                <td>
                                    <?php if($value['value']==1)
                                    {
                                     ...
                                     ?>
                                    <div class="rating">
    <!-- Assuming you want the rating to be associated with each product -->
    <input type="radio" id="star5_<?php echo $value['ID']; ?>" name="rating_<?php echo $value['ID']; ?>" value="5"><label for="star5_<?php echo $value['ID']; ?>"></label>
    <input type="radio" id="star4_<?php echo $value['ID']; ?>" name="rating_<?php echo $value['ID']; ?>" value="4"><label for="star4_<?php echo $value['ID']; ?>"></label>
    <input type="radio" id="star3_<?php echo $value['ID']; ?>" name="rating_<?php echo $value['ID']; ?>" value="3"><label for="star3_<?php echo $value['ID']; ?>"></label>
    <input type="radio" id="star2_<?php echo $value['ID']; ?>" name="rating_<?php echo $value['ID']; ?>" value="2"><label for="star2_<?php echo $value['ID']; ?>"></label>
    <input type="radio" id="star1_<?php echo $value['ID']; ?>" name="rating_<?php echo $value['ID']; ?>" value="1" <?php echo ($value['vote'] == 1) ? 'checked' : ''; ?>><label for="star1_<?php echo $value['ID']; ?>"></label>
</div>

<button class="saveRating" data-productid="<?php echo $value['ID']; ?>">Đánh giá</button>                                    <?php } else if($value['value']==2){
                                        
                                     ?>
                                     <div class="rating">
                                    <?php
                                    $ratingValue = $value['vote']; // Assuming 'rating' is the column in your database holding the rating value
                                    for ($i = 5; $i >= 1; $i--) {
                                        $checked = ($i == $ratingValue) ? 'checked' : '';
                                        echo '<input type="radio" id="star' . $i . '_' . $value['ProductID'] . '" name="rating_' . $value['ProductID'] . '" value="' . $i . '" ' . $checked . ' disabled><label for="star' . $i . '_' . $value['ProductID'] . '" ></label>';
