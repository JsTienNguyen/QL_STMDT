...
<section id="cart">
    <div class="container">
        ...
        if (isset($_SESSION['client']['user'])) {
            ...
            $getoderQuery = "SELECT * FROM oder WHERE Status = 0 AND Email = '$email'";
            $getoderResult = mysqli_query($conn, $getoderQuery);
            $hasoderZero = mysqli_num_rows($getoderResult) > 0;
            if (!$hasoderZero) {
                echo ("<h3 style='text-align: center; color:#846d62; text-transform: uppercase;'>Giỏ hàng của bạn đang trống !</h3>");
                                ...
                                while ($value = $kq1->fetch_assoc()) { ?>
                                    <tr style="border-bottom: 1px solid #C4DFDF;">
                                        <td class="cart_img">
                                            ...
                                            if ($value['discount'] > 0) {
                                                ...
                                                <div class="image_main">
                                                    <img src="./Image/Product/<?php echo $value['Image']; ?>"
                                                        alt="<?php echo $value['Name']; ?>">
                                                    <div class="discount" style="background-color: red;font-size: 14px;">
                                        ...
                                        if ($value['discount'] > 0) {
                                            ?>
                                            <td>
                                                <p style="text-decoration: line-through;" data-productid="<?php echo $value['price']; ?>">
                                                    <?php echo number_format($value['price']); ?>
                                                </p>
                                                <p
                                                    data-productid="<?php echo ($value['price'] - ($value['price'] * $value['discount'] * 0.01)); ?>">
                                                    <?php echo number_format($value['price'] - ($value['price'] * $value['discount'] * 0.01)); ?>
                                                </p>
                                            </td>
                                            </td>
                                            <?php
                                            ...
                                            ?>
                                            <td>
                                                <p data-productid="<?php echo $value['price']; ?>">
                                                    <?php echo number_format($value['price']); ?>
                                                </p>
                                        ...
                                        <td>
                                            <form action="cart.php" method="get">
                                                ...
                                                <input class="cart_quantity_input quantity-input" type="number" name="quantity"
                                        ...
                                        <td class="thanhtien">
                                            <p><span class="total">
                                                    <?php echo number_format(($value['price'] - ($value['price'] * $value['discount'] * 0.01)) * $value['quantity']); ?>
                                                </span></p>
                                        </td>

                                        <td class="delete"><a href="viewcart.php" class="delete-btn"
                                                data-productid="<?php echo $value['ProductID']; ?>"><i
                ...
                <form action="" method="post">
                    <div class="pay">
                        <div class="info">
                            ...
                            <div class="check_info">
                                ...
                                if (isset($_SESSION['client']['user'])) {
                                    ...
                                    $userLoggedIn = true;
                                    $cart = (isset($_SESSION['cart'][$_SESSION['client']['UserID']])) ? $_SESSION['cart'][$_SESSION['client']['UserID']] : [];

                                }
                                ?>
                                <div class="name">
                                    <input type="text" name="tenkh" placeholder="Họ & Tên">
                                    <input type="text" name="sdt" placeholder="Số điện Thoại" value="<?php echo '0' . $sdts; ?>"
                                        readonly>
                                </div>
                                <div class="dress">
                                    ...
                                    <input type="text" name="diachi" placeholder="Địa chỉ của bạn"
                                        value="<?php echo $diachis; ?>" readonly>
                                </div>
                                <textarea style="resize: none;" name="note" id="adress" cols="75" rows="5"
                        ...
                        <table>
                            <tr class="total">
                                <td>Tổng tiền : <span class="tong" id="grand-total">
                                        <?php
                                        $getTTQuery = "SELECT * FROM oder WHERE Status = 0";
                                        $getTTResult = mysqli_query($conn, $getTTQuery);
                                        $TTData = mysqli_fetch_assoc($getTTResult);
                                        $TT = $TTData['total'];
                                        $ID_HD = $TTData['ID'];
                                        echo number_format($TT) ?> VND
                                    </span></td>
                                <?php if ($userLoggedIn): ?>
                                    <td class="thanhtoanthanhcong" data-productid="<?php echo $ID_HD; ?>"><input type="submit"
                                            name="thanhtoan" value="Thanh Toán Đơn Hàng"></td>
                                <?php else: ?>
                                    <td>
                                        <p>Bạn cần đăng nhập để thanh toán đơn hàng.</p>
                                    </td>
                                <?php endif; ?>
                            </tr>
...
<script>
    ...
    function updateTotalPrice(inputElement) {
        ...
        var discountElement = row.querySelector('.discount');

        var price = parseFloat(priceElement.dataset.productid);
        var quantity = parseInt(inputElement.value);

        var discount = parseFloat(discountElement.textContent.replace(/-|%|\s/g, '')) || 0;
